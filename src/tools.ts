...
export const executions = {
  composeTweet: async ({ content }: { content: string }) => {
    console.log("Posting approved tweet");

    const result = await postTweet(content);

    if (result.pending) {
      const statusLine = result.statusUrl
        ? `\n\nStatus: ${result.statusUrl}`
        : result.writeActionId
          ? `\n\nWrite action: ${result.writeActionId}`
          : "";
      return `⏳ Tweet accepted and pending completion.${statusLine}\n\nDo not retry until the write reaches a terminal state.`;
    }
