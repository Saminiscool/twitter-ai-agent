...
async function postTweetWithXquik(content: string): Promise<PostTweetResult> {
  ...
  try {
    ...
    if (response.status === 202) {
      const statusUrl = readStringField(data, "statusUrl");
      const writeActionId = readStringField(data, "writeActionId");
      const statusReference =
        statusUrl || writeActionId || "the returned status";
      ...
      return {
        ...
        statusUrl,
        writeActionId,
        error: `Xquik accepted the write for asynchronous completion. Poll ${statusReference} before retrying.`
      };
    }
    ...
    const tweetId =
      readStringField(data, "tweetId") ||
      readNestedStringField(data, "result", "id");
    return {
      success: true,
      data,
      tweetId,
...
export async function postTweet(content: string): Promise<PostTweetResult> {
  const backend = process.env.X_POST_BACKEND || "twitter";
  if (backend === "xquik") {
  ...
  return {
    success: false,
    error: `Unsupported X_POST_BACKEND: ${backend}. Use "twitter" or "xquik".`
  };
}
